...
#include "part_persist_request.h"
...
struct ompi_part_persist_t {
    ...
    MPI_Comm               part_comm; /* This approach requires a separate tag space, so we need a dedicated communicator. */
    MPI_Request            part_comm_req;
    int32_t                part_comm_ready;
    MPI_Comm               part_comm_setup; /* We create a second communicator to send set-up messages (rational: these 
...
mca_part_persist_progress(void)
{
    ...
    if(0 == ompi_part_persist.init_world && MPI_COMM_WORLD != MPI_COMM_NULL) {
        int err = MPI_Comm_rank(MPI_COMM_WORLD, &ompi_part_persist.my_world_rank);
        if(err != MPI_SUCCESS) {
             exit(-1);
        }
        err = MPI_Comm_idup(MPI_COMM_WORLD, &ompi_part_persist.part_comm, &ompi_part_persist.part_comm_req);
        if(err != MPI_SUCCESS) {
             exit(-1);
        }
        ompi_part_persist.part_comm_ready = 0;
        err = MPI_Comm_idup(MPI_COMM_WORLD, &ompi_part_persist.part_comm_setup, &ompi_part_persist.part_comm_sreq);
        if(err != MPI_SUCCESS) {
            exit(-1);
        }
    ...
    if(0 == ompi_part_persist.init_comms) {
        if(0 == ompi_part_persist.part_comm_ready) {
            MPI_Test(&ompi_part_persist.part_comm_req, &ompi_part_persist.part_comm_ready, MPI_STATUS_IGNORE);
        }
        if(0 == ompi_part_persist.part_comm_sready) {
            MPI_Test(&ompi_part_persist.part_comm_sreq, &ompi_part_persist.part_comm_sready, MPI_STATUS_IGNORE);
        }
    ...
    OPAL_LIST_FOREACH(current, ompi_part_persist.progress_list, mca_part_persist_list_t) {
        ...
        if(false == req->initialized) {
            ...
            if(true == req->flag_post_setup_recv) {
                err = MPI_Irecv(&(req->setup_info[1]), sizeof(struct ompi_mca_persist_setup_t), MPI_BYTE, MPI_ANY_SOURCE, req->my_recv_tag, ompi_part_persist.part_comm_setup, &req->setup_req[1]);
                req->flag_post_setup_recv = false;
            } 
 
            MPI_Test(&(req->setup_req[1]), &done, MPI_STATUS_IGNORE);
            if(done) {
                int32_t dt_size;
                ...
                if(MCA_PART_PERSIST_REQUEST_PSEND == req->req_type) {
                    ...
                    int32_t bytes = req->req_count * dt_size;

                    /* Set up persistant sends */
                    req->persist_reqs = (MPI_Request*) malloc(sizeof(MPI_Request)*(req->real_parts));
                    for(i = 0; i < req->real_parts; i++) {
                         void *buf = ((void*) (((char*)req->req_addr) + (bytes * i)));
                         MPI_Send_init(buf, req->real_count, req->req_datatype, req->world_peer, req->my_send_tag+i, ompi_part_persist.part_comm, &(req->persist_reqs[i]));
                    }    
                } else {
                    /* parse message */
                    req->world_peer  = req->setup_info[1].world_rank; 
                    ...
                    req->real_dt     = req->setup_info[1].dt;

                    err = MPI_Type_size(req->real_dt, &dt_size);
                    if(MPI_SUCCESS != err) return OMPI_ERROR;
                    ...
                    for(i = 0; i < req->real_parts; i++) {
                         void *buf = ((void*) (((char*)req->req_addr) + (bytes * i)));
                         err = MPI_Recv_init(buf, req->real_count, req->real_dt, req->world_peer, req->my_send_tag+i, ompi_part_persist.part_comm, &(req->persist_reqs[i]));
                         err = MPI_Start(&(req->persist_reqs[i]));
                         req->flags[i] = 0;
                    }                     

                    /* Send back a message */
                    req->setup_info[0].world_rank = ompi_part_persist.my_world_rank;
                    err = MPI_Isend(&(req->setup_info[0]), sizeof(struct ompi_mca_persist_setup_t), MPI_BYTE, req->world_peer, req->my_recv_tag, ompi_part_persist.part_comm_setup, &req->setup_req[0]);
                    if(MPI_SUCCESS != err) return OMPI_ERROR;
                } 
            }
        } else {
            if(false == req->req_part_complete && REQUEST_COMPLETED != req->req_ompi.req_complete && OMPI_REQUEST_ACTIVE == req->req_ompi.req_state) {
               for(i = 0; i < req->real_parts; i++) {
                    ...
                    if(0 == req->flags[i])
                    {
                        MPI_Test(&(req->persist_reqs[i]), &(req->flags[i]), MPI_STATUS_IGNORE);
                        ...
                        struct ompi_request_t **request)
{
    ...
    err	= MPI_Irecv(&req->setup_info[1], sizeof(struct ompi_mca_persist_setup_t), MPI_BYTE, src, tag, comm, &req->setup_req[1]); 
    if(MPI_SUCCESS != err) return OMPI_ERROR;
                        ...
                        ompi_request_t** request)
{
    ...
    req->setup_info[0].dt = datatype;
    req->real_dt = datatype;


    req->flags = (int*) calloc(req->real_parts, sizeof(int));

    err = MPI_Isend(&(req->setup_info[0]), sizeof(struct ompi_mca_persist_setup_t), MPI_BYTE, dst, tag, comm, &req->setup_req[0]);
    if(MPI_SUCCESS != err) return OMPI_ERROR;
...
mca_part_persist_start(size_t count, ompi_request_t** requests)
{
    ...
    for(i = 0; i < _count && MPI_SUCCESS == err; i++) {
        ...
        if(false == req->first_send)
        {
            if(MCA_PART_PERSIST_REQUEST_PSEND == req->req_type) {
                req->done_count = 0;
                for(i = 0; i < req->real_parts && MPI_SUCCESS == err; i++) {
                    req->flags[i] = 0;
                }
            } else {
                req->done_count = 0;
                    ...
                    ompi_request_t* request)
{
    ...
    {
        for(i = min_part; i <= max_part && MPI_SUCCESS == err; i++) {
            err = MPI_Start(&req->persist_reqs[i]);
